...
function getRectangleString(width, height) {
  ...
  const sideEdges = `│${' '.repeat(width - 2)}│\n`;
  const bottomEdge = `└${'─'.repeat(width - 2)}┘\n`;

  return `${topEdge}${sideEdges.repeat(height - 2)}${bottomEdge}`;
}
...
function encodeToRot13(str) {
  ...
  const arr = str.split('');
  return arr.map((item) => output[input.indexOf(item)]).join('');
}
...
function isString(value) {
  return ((value instanceof String) && (typeof value === 'object')) ? true : (typeof value === 'string');
}
